...
//    private final ReferencedTypeResolver referencedTypeResolver;
    ...
    @Autowired
    public DirtyServerAttributeMapper(@Value("${api.rest.baseurl}") final String baseUrl) {
    ...
    public Collection<Attribute> map(final RpslAttribute rpslAttribute, final String source) {
        ...
        if (cleanValues.size() == 1) {
            ...
            final String referencedType = null; //(rpslAttribute.getType() != null) ? referencedTypeResolver.getReferencedType(rpslAttribute.getType(), cleanValue) : null;
            final Link link = null; //(referencedType != null) ? createLink(source, referencedType, cleanValue.toString()) : null;
