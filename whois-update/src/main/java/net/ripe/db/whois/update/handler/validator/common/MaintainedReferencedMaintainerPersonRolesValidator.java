...
public class MaintainedReferencedMaintainerPersonRolesValidator extends AbstractObjectIsMaintainedValidator {
    ...
    public void validate(final PreparedUpdate update, final UpdateContext updateContext) {
        ...
        for (final CIString value : updatedObject.getValuesForAttribute(AttributeTypes.MNT_BY)) {
            ...
            try {
                ...
                for (RpslObject rpslObject : validateReferencedPersonsAndRoles(object)) {
                    updateContext.addMessage(update, UpdateMessages.referencedObjectMissingAttribute(rpslObject.getType(), rpslObject.getKey(), ObjectType.MNTNER, value, AttributeTypes.MNT_BY));
                }
    ...
    private boolean hasReferenceToPersonRole(final RpslObject object) {
        final List<RpslAttribute> attributes = object.findAttributes(AttributeTypes.ADMIN_C, AttributeTypes.TECH_C, AttributeTypes.ZONE_C, AttributeTypes.PING_HDL, AttributeTypes.AUTHOR, AttributeTypes.ABUSE_C);
