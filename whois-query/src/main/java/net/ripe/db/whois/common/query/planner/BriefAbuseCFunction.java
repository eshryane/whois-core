...
class BriefAbuseCFunction implements Function<ResponseObject, ResponseObject> {
    private static final Set<AttributeType> BRIEF_ATTRIBUTES = Sets.newHashSet(AttributeTypes.INETNUM, AttributeTypes.INET6NUM, AttributeTypes.AUT_NUM, AttributeTypes.ABUSE_MAILBOX);
    ...
    public ResponseObject apply(final @Nullable ResponseObject input) {
        ...
        if (Query.ABUSE_CONTACT_OBJECT_TYPES.contains(rpslObject.getType())) {
            ...
            if (abuseContact != null) {
                ...
                abuseCAttributes.add(new RpslAttribute(AttributeTypes.ABUSE_MAILBOX, abuseContact));
