...
public class SearchQueryExecutorTest {
    ...
    public void all_attributes_handled() {
        for (final AttributeType attributeType : AttributeType.values()) {
            assertTrue(subject.supports(new Query("-i " + attributeType.getName() + " query", Query.Origin.LEGACY, false, queryMessages)));
        }
    }

    @Test(expected = QueryException.class)
    public void test_supports_no_attributes() {
        assertThat(subject.supports(new Query("-i", Query.Origin.LEGACY, false, queryMessages)), is(false));
    }

    @Test
    public void test_supports_inverse_with_filter() {
        assertThat(subject.supports(new Query("-T inetnum -i mnt-by aardvark-mnt", Query.Origin.LEGACY, false, queryMessages)), is(true));
    }

    @Test
    public void test_supports_inverse_recursive() {
        assertThat(subject.supports(new Query("-i mnt-by aardvark-mnt", Query.Origin.LEGACY, false, queryMessages)), is(true));
    }

    @Test
    public void test_supports_inverse() {
        assertThat(subject.supports(new Query("-r -i mnt-by aardvark-mnt", Query.Origin.LEGACY, false, queryMessages)), is(true));
    }

    @Test
    public void test_supports_inverse_multiple() {
        assertThat(subject.supports(new Query("-r -i mnt-by,mnt-ref aardvark-mnt", Query.Origin.LEGACY, false, queryMessages)), is(true));
    }

    @Test
    public void test_supports_inverse_multiple_unknown() {
        assertThat(subject.supports(new Query("-r -i mnt-by,mnt-ref,mnt-lower aardvark-mnt", Query.Origin.LEGACY, false, queryMessages)), is(true));
    }

    @Test
    public void unknown_source() {
        final Query query = new Query("-s UNKNOWN 10.0.0.0", Query.Origin.LEGACY, false, queryMessages);
    ...
    public void query_all_sources() {
        ...
        final Query query = new Query("--all-sources 10.0.0.0", Query.Origin.LEGACY, false, queryMessages);
        final CaptureResponseHandler responseHandler = new CaptureResponseHandler();
        subject.execute(query, responseHandler);
    ...
    public void query_sources() {
        final Query query = new Query("--sources APNIC-GRS,ARIN-GRS 10.0.0.0", Query.Origin.LEGACY, false, queryMessages);
    ...
    public void query_sources_and_additional() {
        ...
        final Query query = new Query("--all-sources --sources RIPE 10.0.0.0", Query.Origin.LEGACY, false, queryMessages);
        final CaptureResponseHandler responseHandler = new CaptureResponseHandler();
        subject.execute(query, responseHandler);
    ...
    public void query_resources() {
        ...
        final Query query = new Query("--resource 10.0.0.0", Query.Origin.LEGACY, false, queryMessages);
        final CaptureResponseHandler responseHandler = new CaptureResponseHandler();
        subject.execute(query, responseHandler);
    ...
    public void no_results_found_gives_message() {
        final Query query = new Query("-s RIPE 10.0.0.0", Query.Origin.LEGACY, false, queryMessages);
